#include "melody.h"
#include <stdint.h>
extern void play_note(float freq,uint16_t duration,double vol);


//// Define note frequencies based on A4 = 440 Hz.
//const double C4 = 261.63;
//const double D4 = 293.66;
//const double E4 = 329.63;
//const double G4 = 392.00;

const double A4 = 440.00;
const double B4 = 493.88;
const double C5 = 523.25;
const double D5 = 587.33;
const double E5 = 659.26;
const double F5 = 698.46;
const double G5 = 783.99;
const double A5 = 880.00;
const double C4 = 261.63;
const double D4 = 293.66;
const double E4 = 329.63;
const double F4 = 369.99;
const double G4 = 392.00;
const double REST = 0.0;
// Define base note durations for a tempo of 120 BPM
const double EIGHTH_NOTE = 0.25;
const double QUARTER_NOTE = 0.5;
const double HALF_NOTE = 1.0;

// Define base volumes for dynamics.
const double VOLUME_PIANO = 0.6;
const double VOLUME_MEZZO_PIANO = 0.7;
const double VOLUME_MEZZO_FORTE = 0.8;
const double VOLUME_FORTE = 1.0;
const double VOLUME_SILENCE = 0.0;

// An array of Note structs representing the expressive melody.
const melody_s mary_lamb_melody[] = {
    // "Ma-ry had a lit-tle lamb"
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {C4, QUARTER_NOTE, VOLUME_PIANO, NORMAL}, // Softer "had"
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_FORTE, LEGATO},   // Louder, connected
    {E4, QUARTER_NOTE, VOLUME_FORTE, LEGATO},   // Louder, connected
    {E4, HALF_NOTE, VOLUME_FORTE, NORMAL},

    // "lit-tle lamb, lit-tle lamb"
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, STACCATO}, // Detached
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, STACCATO}, // Detached
    {D4, HALF_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {G4, QUARTER_NOTE, VOLUME_FORTE, LEGATO},
    {G4, HALF_NOTE, VOLUME_FORTE, NORMAL},

    // "Ma-ry had a lit-tle lamb"
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {C4, QUARTER_NOTE, VOLUME_PIANO, NORMAL}, // Softer "had"
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_FORTE, LEGATO},
    {E4, QUARTER_NOTE, VOLUME_FORTE, LEGATO},
    {E4, HALF_NOTE, VOLUME_FORTE, NORMAL},

    // "its fleece was white as snow"
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D4, QUARTER_NOTE, VOLUME_PIANO, STACCATO},
    {C4, HALF_NOTE, VOLUME_PIANO, NORMAL} // Final note is soft
};

const melody_s star_wars_theme[] = {
    // Measures 1-2
    {G4, HALF_NOTE + EIGHTH_NOTE, VOLUME_FORTE, NORMAL},
    {C5, EIGHTH_NOTE, VOLUME_FORTE, STACCATO},
    {G5, QUARTER_NOTE, VOLUME_FORTE, NORMAL},
    {F5, EIGHTH_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E5, EIGHTH_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D5, EIGHTH_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {C5, EIGHTH_NOTE, VOLUME_FORTE, NORMAL},
    {A4, HALF_NOTE, VOLUME_MEZZO_FORTE, NORMAL},

    // Measures 3-4
    {G5, QUARTER_NOTE, VOLUME_FORTE, NORMAL},
    {F5, EIGHTH_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E5, EIGHTH_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D5, EIGHTH_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {C5, EIGHTH_NOTE, VOLUME_FORTE, NORMAL},
    {A4, HALF_NOTE, VOLUME_MEZZO_FORTE, NORMAL},

    // Measures 5-6 (The "Force" theme, simplified)
    {G4, QUARTER_NOTE, VOLUME_PIANO, NORMAL},
    {C5, QUARTER_NOTE, VOLUME_PIANO, NORMAL},
    {D5, HALF_NOTE, VOLUME_MEZZO_PIANO, NORMAL},
    {REST, QUARTER_NOTE, VOLUME_SILENCE, NORMAL},

    // Measures 7-8 (The "Rebel Fanfare"
    {E5, QUARTER_NOTE, VOLUME_FORTE, NORMAL},
    {A4, QUARTER_NOTE, VOLUME_FORTE, NORMAL},
    {E5, QUARTER_NOTE, VOLUME_FORTE, NORMAL},
    {D5, QUARTER_NOTE, VOLUME_FORTE, NORMAL},

    // Measures 9-10
    {C5, QUARTER_NOTE, VOLUME_FORTE, NORMAL},
    {B4, QUARTER_NOTE, VOLUME_FORTE, NORMAL},
    {A4, HALF_NOTE, VOLUME_FORTE, NORMAL},
    {REST, QUARTER_NOTE, VOLUME_SILENCE, NORMAL},
};

//const melody_s twinkle_twinkle_melody[] = {
//    // "Twin-kle, twin-kle, lit-tle star"
//    {C4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {C4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {G4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {G4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {A4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {A4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {G4, HALF_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {REST, QUARTER_NOTE, VOLUME_SILENCE, NORMAL}, // Rest
//
//    // "How I won-der what you are"
//    {F4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {F4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {C4, HALF_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
//    {REST, QUARTER_NOTE, VOLUME_SILENCE, NORMAL}, // Rest
//};

const melody_s twinkle_twinkle_melody_complete[] = {
    // Section A: "Twinkle, twinkle, little star"
    {C4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {C4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {G4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {G4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {A4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {A4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {G4, HALF_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {REST, QUARTER_NOTE, VOLUME_SILENCE, NORMAL},

    // Section A: "How I wonder what you are"
    {F4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {F4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {C4, HALF_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {REST, QUARTER_NOTE, VOLUME_SILENCE, NORMAL},

    // Section B: "Up above the world so high"
    {G4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {G4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {F4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {F4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D4, HALF_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {REST, QUARTER_NOTE, VOLUME_SILENCE, NORMAL},

    // Section B: "Like a diamond in the sky"
    {G4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {G4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {F4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {F4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D4, HALF_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {REST, QUARTER_NOTE, VOLUME_SILENCE, NORMAL},

    // Section A: "Twinkle, twinkle, little star"
    {C4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {C4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {G4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {G4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {A4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {A4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {G4, HALF_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {REST, QUARTER_NOTE, VOLUME_SILENCE, NORMAL},

    // Section A: "How I wonder what you are"
    {F4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {F4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {E4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {D4, QUARTER_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {C4, HALF_NOTE, VOLUME_MEZZO_FORTE, NORMAL},
    {REST, QUARTER_NOTE, VOLUME_SILENCE, NORMAL},
};

void play_melody(const melody_s* mel,int len)
{
	for(int i=0;i<len;i++)
	{
		play_note( mel[i].frequency , mel[i].duration * 1000 , mel[i].volume );
	}
}
